...
graph TD
    ...
    subgraph "三区安全路由决策"
        Router -->|"<54 或 >250 mg/dL"| Red[🔴 红区: 拦截叙事并返回硬编码急救指引 + 就医模板]
        Router -->|"<70 或 >180 mg/dL"| Yellow[🟡 黄区: 前缀安全警示 + 正常生成叙事]
...
在 LLM 叙事层前置对当前血糖时序数据进行分区拦截。阈值与代码常量一一对应（`RED_ZONE_LOW/HIGH = 54/250`、`YELLOW_ZONE_LOW/HIGH = 70/180`，均为严格不等式，恰好等于阈值的读数归上一级较安全区；对齐 AGP 2019 国际共识与 `metrics.py` 的 180 mg/dL 高阈值）：
*   🟢 **绿区 (70 ≤ 值 ≤ 180 mg/dL)**：安全，正常进行生成与总结。
*   🟡 **黄区 (54 ≤ 值 < 70 或 180 < 值 ≤ 250 mg/dL)**：偏低/偏高，在正常消息前方添加 `⚠️ 提示前缀`，叙事不中断。低侧（ADA Level 1 低血糖）前缀内嵌 15-15 法则自助指引；高侧对应"目标范围外但未达 Level 2 高血糖界"。
*   🔴 **红区 (<54 或 >250 mg/dL)**：**强硬编码拦截**。立刻截断并废弃 LLM 的叙事内容生成，按方向返回固定急救指引（低侧：速效碳水/胰高血糖素/联系急救；高侧：测酮体/补水/持续不降就医）+ "无法代替医生"的转诊尾句。指引为确定性模板文本（ADA 公开标准处置步骤），非 LLM 生成。红区判定有 10 分钟持续门控（瞬时毛刺降级黄区），红区事件后 2 小时内自动执行恢复复查（见 §5c）。
