...
- **Test baseline (T001, 2026-06-09)**: ≥407 tests green (post-F4 baseline). Verified 2026-06-10: 407 green locally after installing the `tzdata` PyPI package (Windows lacks the IANA tz database that `zoneinfo` needs — env-only fix, no code change). Post-Foundational (T002–T004): 410 green (+3 guard tests).
...
- **Analyze remediation (2026-06-10)**: (U1) Citation guard in `builder.py` must run only on the medical narrative section, not the deterministic analytics section. (U2) `SafetyRouter` must be instantiated once per process (or injected via DI) so the 2-hour recovery window state is preserved across `evaluate` calls. (I1) Template names must be distinct: `CITATION_BLOCK_TEMPLATE` in `renderer.py` and `RED_ZONE_TEMPLATE` in `router.py`. (I3) Updated test baseline to 407. (L1) If `recovery_check is None`, the renderer must skip rendering the recovery segment.
