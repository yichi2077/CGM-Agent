...
class TestGreenZone:
    def test_normal_values_return_clear(self):
        points = [_point(100), _point(120), _point(90)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "clear"
        assert decision.route == "reports.generate"
        assert decision.message is None

    def test_boundary_values_still_green(self):
        """Exactly at yellow thresholds should still be green (strict < / >)."""
        points = [_point(YELLOW_ZONE_LOW_MGDL), _point(YELLOW_ZONE_HIGH_MGDL)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "clear"

    def test_empty_points_returns_green(self):
        decision = SafetyRouter().evaluate(scope=_scope(), points=[])
        assert decision.safety_result["status"] == "clear"


class TestYellowZone:
    def test_low_yellow_detected(self):
        points = [_point(65)]  # below 70
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "yellow_zone"
        assert decision.safety_result["direction"] == "偏低"
        assert decision.message is not None
        assert "偏低" in decision.message

    def test_high_yellow_detected(self):
        points = [_point(260)]  # above 250 but below 300
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "yellow_zone"
        assert decision.safety_result["direction"] == "偏高"

    def test_yellow_uses_mg_dl_not_raw_value(self):
    ...
    def test_yellow_evidence_refs_use_mg_dl(self):
        points = [_point(65)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.evidence_refs is not None
        ref = decision.evidence_refs[0]
        assert ref.kind == EvidenceKind.GLUCOSE_POINT
        assert "mg/dL" in ref.summary

    def test_yellow_route_is_reports_generate(self):
...
class TestRedZone:
    def test_low_red_detected(self):
        points = [_point(50)]  # below 54
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "red_zone"
        assert decision.route == "reports.generate.red_zone"

    def test_high_red_detected(self):
        points = [_point(350)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert decision.safety_result["status"] == "red_zone"

    def test_red_uses_mg_dl_not_raw_value(self):
    ...
    def test_red_min_max_in_mg_dl(self):
        points = [_point(40), _point(350)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        result = decision.safety_result
        assert result["min_value_mgdl"] == 40.0
        assert result["max_value_mgdl"] == 350.0

    def test_red_min_max_with_mmol_input(self):
        ...
        assert result["min_value_mgdl"] < RED_ZONE_LOW_MGDL
        assert result["max_value_mgdl"] > RED_ZONE_HIGH_MGDL

    def test_red_message_is_medical_deferral(self):
        points = [_point(50)]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert "医疗判断" in decision.message

    def test_red_evidence_refs_capped_at_5(self):
        points = [_point(v) for v in [40, 42, 44, 46, 48, 50, 52]]
        decision = SafetyRouter().evaluate(scope=_scope(), points=points)
        assert len(decision.evidence_refs) == 5
